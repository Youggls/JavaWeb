...
1. 下载我们的[Release文件](https://github.com/Youggls/JavaWeb/releases)
2. 将war包放到Tomcat的Webapp目录下
3. 启动Tomcat，然后停止
4. 到webapps目录下找到`./JavaWeb/WEB-INF/classes/db_design`文件夹，将里边的sql文件执行一次（先执行最外部`dbEnity.sql`）
5. 修改`./JavaWeb/WEB-INF/classes/bin/config.json`，前两个字段是数据库用户名和用户密码，后两个字段分别为Java连接数据库的URL和图片存储在服务器上的路径
6. 再次启动Tomcat服务！
7. 成功！
...
- [x] **TextView，用来浏览帖子的视图（前端，RaymondY）**
- [x] **用来浏览posts的图，类似知乎热榜**（半完成，按时间，按关注都可以了，只差按热度，好说）
- [x] **搜索功能（前后端）**
- [ ] 做个首页的图（RaymodY）
- [x] 修改一下登录注册时按钮长度（前端，RaymondY）
- [x] ~~文本编辑， 紧急（前端，DoubleClass）~~
- [ ] ~~锁死前端的显示，取消自适应，类似知乎~~（待议）
...
GitHubID|分工
---|---
[@Youggls](https://github.com/Youggls)|后端、数据库设计，协助开发前端|
[@Raymond](https://github.com/Raymond)|前端设计|
[@DoubleClass](https://github.com/DoubleClass)|后端、数据库设计，协助开发前端|
