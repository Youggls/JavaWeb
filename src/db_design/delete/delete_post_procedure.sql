...
begin
    ...
    declare curForComment cursor for select floor_id from floor where parent_post_id = mpost_id;
    declare continue handler for  SQLSTATE '02000' set myFLoor_id=0;
    
    select count(*) from floor where parent_post_id = mpost_id into numOfFloor;
    open curForComment;
